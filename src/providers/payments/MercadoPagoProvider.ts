...
export class MercadoPagoProvider implements PaymentProvider {
  ...
  async connect(): Promise<boolean> {
    console.log('[MercadoPagoProvider] Conectando...');
    return true;
  }

  async disconnect(): Promise<boolean> {
    console.log('[MercadoPagoProvider] Desconectando...');
    return true;
  }

  async createPixCharge(vendaId: string, clienteId: string, clienteNome: string, valor: number): Promise<PixCharge> {
    const txid = 'mp_' + Math.random().toString(36).substring(2, 12).toUpperCase();
    const copiaECola = `00020101021226780014br.gov.bcb.pix2556pix.mercadopago.com/v2/${txid}5204000053039865405${valor.toFixed(2)}5802BR5915CantinaSmartPOS6009Sao Paulo62070503***63048D52`;
  ...
  async cancelCharge(txid: string): Promise<boolean> {
    console.log(`[MercadoPagoProvider] Cancelando cobrança ${txid}`);
    return true;
  }

  async checkPayment(txid: string): Promise<'Aguardando pagamento' | 'Pago' | 'Cancelado' | 'Expirado' | 'Estornado' | 'Parcial'> {
    console.log(`[MercadoPagoProvider] Verificando pagamento ${txid}`);
    return 'Aguardando pagamento';
  }

  async receiveWebhook(payload: any): Promise<{ success: boolean; txid: string; valorPago: number; status: string }> {
    console.log('[MercadoPagoProvider] Webhook recebido', payload);
    const txid = payload.data?.id || payload.txid || '';
  ...
  async refund(txid: string, valor?: number): Promise<boolean> {
    console.log(`[MercadoPagoProvider] Estornando ${valor || 'total'} para txid ${txid}`);
    return true;
  }
  ...
  async generatePixCopyPaste(txid: string, valor: number): Promise<string> {
    return `00020101021226780014br.gov.bcb.pix2556pix.mercadopago.com/v2/${txid}5204000053039865405${valor.toFixed(2)}5802BR5915CantinaSmartPOS6009Sao Paulo62070503***63048D52`;
  }
