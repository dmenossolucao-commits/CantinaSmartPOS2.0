...
export class EfiProvider implements PaymentProvider {
  ...
  async connect(): Promise<boolean> {
    console.log('[EfiProvider] Conectando...');
    return true;
  }

  async disconnect(): Promise<boolean> {
    console.log('[EfiProvider] Desconectando...');
    return true;
  }

  async createPixCharge(vendaId: string, clienteId: string, clienteNome: string, valor: number): Promise<PixCharge> {
    const txid = 'efi_' + Math.random().toString(36).substring(2, 12).toUpperCase();
    const copiaECola = `00020101021226730014br.gov.bcb.pix2551pix.efibank.com.br/v2/${txid}5204000053039865405${valor.toFixed(2)}5802BR5915CantinaSmartPOS6009Sao Paulo62070503***6304D1A9`;
  ...
  async cancelCharge(txid: string): Promise<boolean> {
    console.log(`[EfiProvider] Cancelando cobrança ${txid}`);
    return true;
  }

  async checkPayment(txid: string): Promise<'Aguardando pagamento' | 'Pago' | 'Cancelado' | 'Expirado' | 'Estornado' | 'Parcial'> {
    console.log(`[EfiProvider] Verificando pagamento ${txid}`);
    return 'Aguardando pagamento';
  }

  async receiveWebhook(payload: any): Promise<{ success: boolean; txid: string; valorPago: number; status: string }> {
    console.log('[EfiProvider] Webhook recebido', payload);
    const pixData = payload.pix?.[0] || {};
    const txid = pixData.txid || payload.txid || '';
    const valorPago = parseFloat(pixData.valor || payload.valor) || 0;
    return {
      success: true,
      txid,
      valorPago,
  ...
  async refund(txid: string, valor?: number): Promise<boolean> {
    console.log(`[EfiProvider] Estornando ${valor || 'total'} para txid ${txid}`);
    return true;
  }
  ...
  async generatePixCopyPaste(txid: string, valor: number): Promise<string> {
    return `00020101021226730014br.gov.bcb.pix2551pix.efibank.com.br/v2/${txid}5204000053039865405${valor.toFixed(2)}5802BR5915CantinaSmartPOS6009Sao Paulo62070503***6304D1A9`;
  }
